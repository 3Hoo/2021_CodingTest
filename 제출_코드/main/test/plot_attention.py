...
test_loader = dm.train_data_loader_DSA(TEST_DATA, batch_size=batch_size, device="cuda", use_angle_v=args.bUseAngleV, shuffle=False)

# 총 315개의 test data, 20개 마다의 텀을 주어서 15장의 사진을 출력하도록 해보자
img_save_term = 20

for self_idx, (cur_pos, next_pos, prev_differ, x, target) in enumerate(test_loader) : 
    dx_ = model_d(x, prev_differ[:,:,0])
    dy_ = model_d(x, prev_differ[:,:,1])
    
    dx_ = dx_.squeeze().detach().cpu().numpy()
    dy_ = dy_.squeeze().detach().cpu().numpy()
    
    real_pos = np.array([next_pos[:,0], next_pos[:,1]])
    pred_pos = np.array([cur_pos[:,0,-1]+dx_.squeeze().item(), cur_pos[:,1,-1]+dy_.squeeze().item()])
    ...
    if self_idx % img_save_term == 0 :
        plt.cla()
        plt.subplot(1, 1, 1)
        plt.scatter(real_pos[0,:], real_pos[1,:], c='g', s=80, label="Real Next Pos")
        plt.scatter(pred_pos[0,:], pred_pos[1,:], c='r', s=80, label="Predicted Next Pos")
        plt.plot([real_pos[0,:], pred_pos[0,:]], [real_pos[1,:], pred_pos[1,:]], c='darkviolet', label="euclidean Distance")
        txt = plt.text(x=(real_pos[0,:]+pred_pos[0,:])/2, y=(real_pos[1,:]+pred_pos[1,:])/2 + 50, s=str(euclidean_distance), c='yellow', size='large')
        txt.set_path_effects([PathEffects.withStroke(linewidth=5, foreground='black')])
        plt.scatter(cur_pos[:,0,:], cur_pos[:,1,:], c='b', s=60, label="Path")
        plt.legend()
