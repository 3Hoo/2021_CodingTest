...
for self_idx, (dx, dy, dirx, diry, linx, liny, c, n, t) in enumerate(test_loader) : 
    ...
    if self_idx % img_save_term == 0 :
        ...
        txt = plt.text(x=(real_pos[0,:]+pred_pos[0,:])/2, y=(real_pos[1,:]+pred_pos[1,:])/2 + 50, s=str(euclidian_distance), c='yellow', size='large')
        txt.set_path_effects([PathEffects.withStroke(linewidth=5, foreground='black')])
        plt.scatter(c[:,0,:].detach().cpu().numpy(), c[:,1,:].detach().cpu().numpy(), c='b', s=60, label="Path")
        plt.legend()
