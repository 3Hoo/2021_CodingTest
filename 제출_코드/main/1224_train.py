...
for epoch in range(args.epochs+1) : 
    ...
    train_loader = dm.train_data_loader_DSA(TRAIN_DATA, batch_size=batch_size, device="cuda", use_angle_v=args.bUseAngleV, shuffle=False)
    
    for self_idx, (cur_pos, next_pos, prev_differ, x, target) in enumerate(train_loader) : 
        total_loss = 0.0
    ...
    dev_loader = dm.train_data_loader_DSA(VAL_DATA, batch_size=batch_size, device="cuda", use_angle_v=args.bUseAngleV, shuffle=False)
    
    with torch.no_grad() :
        for self_idx, (cur_pos, next_pos, prev_differ, x, target) in enumerate(dev_loader) :
            total_loss = 0.0
